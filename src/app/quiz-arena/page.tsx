...
const StartButton = withSubscriptionGate(
  ({ onClick, isLoading }: { onClick: () => void; isLoading?: boolean }) => (
    <Button
      onClick={onClick}
      disabled={isLoading}
      className="rounded-full px-10 py-6 bg-primary text-primary-foreground hover:bg-primary/90 text-base font-semibold"
    >
  ...
  const handleStart = async () => {
    ...
    if (user) {
      setUser({
        ...user,
        usage: user.usage
          ? { ...user.usage, quizzesCount: user.usage.quizzesCount + 1 }
          : null,
      });
    }
    setPhase("question");
      ...
      <AnimatePresence mode="wait">
        {phase === "lobby" && (
          ...
          >
            <motion.div
              variants={item}
              className="md:col-span-2 rounded-3xl border border-primary/10 bg-card p-10 flex flex-col gap-6 relative overflow-hidden"
            >
              ...
              <div className="relative">
                ...
                <p className="text-muted-foreground text-lg leading-relaxed">
                  Test your mastery across three critical pillars:{" "}
                  ...
                  . Each question has a 30-second Shot Clock. Immediate feedback
                  on every answer.
                </p>
            ...
            <motion.div variants={item} className="flex flex-col gap-4">
              {[
                ...
                <div
                  key={cat}
                  className="rounded-3xl border border-primary/10 bg-card p-6 flex items-center gap-4 group hover:border-primary/30 transition-colors"
                >
        ...
        {phase === "results" && (
          ...
          >
            <motion.div
              variants={item}
              className="md:col-span-2 rounded-3xl border border-primary/10 bg-card p-10 flex flex-col gap-6"
            >
              ...
              <div className="flex flex-col gap-4">
                {questions.map((q, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-4 p-5 rounded-2xl border border-border bg-muted/20"
                  >
                    {answers[i] ? (
            ...
            <motion.div variants={item} className="flex flex-col gap-4">
              <div className="rounded-3xl border border-primary/10 bg-card p-8 flex flex-col items-center text-center gap-4">
                ...
                <span className="text-7xl font-playfair font-black text-primary">
                  {pct}%
                </span>
                <p className="text-muted-foreground">
