...
import {
  FileText, Trophy, Clock, Target, ArrowRight,
  Plus, Calendar, MapPin, X, Trash2, ChevronDown, Check,
} from "lucide-react";
...
const modal: Variants = {
  ...
  show: { opacity: 1, scale: 1, y: 0, transition: { duration: 0.25, ease: [0.25,0.1,0.25,1] as [number,number,number,number] } },
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-muted-foreground text-sm mt-0.5">
                {initial?.id ? "Update your assignment details." : "Set up your upcoming assignment."}
              </p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium">Conference Name<span className="text-primary ml-0.5">*</span></label>
              <Input placeholder="e.g. Global MUN Summit 2026" value={form.name}
                onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                className="rounded-2xl border-border h-11" required />
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <Input placeholder="e.g. New York, USA" value={form.location}
                  onChange={(e) => setForm((f) => ({ ...f, location: e.target.value }))}
                  className="rounded-2xl border-border h-11" />
              </div>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium">Committee<span className="text-primary ml-0.5">*</span></label>
              <Input placeholder="e.g. UN Security Council" value={form.committee}
                onChange={(e) => setForm((f) => ({ ...f, committee: e.target.value }))}
                className="rounded-2xl border-border h-11" required />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium">Country Representing<span className="text-primary ml-0.5">*</span></label>
              <Input placeholder="e.g. France" value={form.country}
                onChange={(e) => setForm((f) => ({ ...f, country: e.target.value }))}
                className="rounded-2xl border-border h-11" required />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium">Topic / Agenda Item</label>
              <Input placeholder="e.g. Cybersecurity Threats" value={form.topic}
            ...
            <div className="flex gap-3 mt-2">
              ...
              <Button type="submit" className="flex-1 rounded-full bg-primary text-primary-foreground hover:bg-primary/90">
...
function ConferenceSwitcher() {
  const { conferences, activeId, setActiveId, deleteConference } = useConferenceStore();
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="max-w-[180px] truncate">{active?.name ?? "Select conference"}</span>
            ...
            <div className="p-2 flex flex-col gap-0.5 max-h-64 overflow-y-auto">
              {conferences.map((c) => (
                ...
                >
                  ...
                  >
                    <p className={`text-sm font-medium truncate ${c.id === activeId ? "text-primary" : "text-foreground"}`}>
                      {c.name}
                    </p>
                    {c.committee && (
                      <p className="text-xs text-muted-foreground truncate">{c.committee}</p>
                    )}
                  </div>
                  {c.id === activeId && <Check className="w-4 h-4 text-primary shrink-0" />}
                  <button
                    onClick={(e) => { e.stopPropagation(); deleteConference(c.id); }}
  ...
  return (
    ...
    >
      <motion.div variants={item}>
        ...
        <h3 className="font-playfair text-3xl font-bold mb-3">No conferences yet</h3>
        <p className="text-muted-foreground text-lg max-w-md mx-auto">
          Add your first conference and your Command Center will tailor every module — speeches, quizzes, debates — to your specific committee and country.
        </p>
      </motion.div>

      <motion.div variants={item}>
        <Button onClick={onAdd} className="rounded-full px-8 py-6 text-base font-semibold gap-2">
          <Plus className="w-4 h-4" /> Add Your First Conference
        </Button>
      </motion.div>

      <motion.div variants={item} className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-2xl">
        {[
...
function ActiveDashboard({
  conference,
  onEdit,
}: { conference: Conference; onEdit: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 flex flex-col gap-4">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <h3 className="font-geist font-semibold text-2xl">{conference.name}</h3>
              <p className="text-primary font-medium mt-0.5">Active assignment</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            {[
              { icon: Clock,  label: "Date",         value: fmtDate(conference.date), sub: days !== null && days > 0 ? `${days}d away` : undefined },
              { icon: Target, label: "Location",     value: conference.location || "—" },
              { icon: Trophy, label: "Committee",    value: conference.committee },
              { icon: Target, label: "Representing", value: conference.country },
          ...
          {conference.topic && (
            <div className="bg-background/50 rounded-2xl p-5 border border-border/50">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="font-semibold text-sm">Agenda Topic</span>
              </div>
              <p className="text-foreground">{conference.topic}</p>
            </div>
          )}
        </div>
      </motion.div>
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Use the modules to track your prep for <span className="font-medium text-foreground">{conference.country}</span> in <span className="font-medium text-foreground">{conference.committee}</span>.
          </p>
          {/* Progress bars — real data comes from backend */}
          <div className="mt-6 flex flex-col gap-4">
            {["Speeches drafted", "Crisis directives", "Quiz sessions"].map((label) => (
      ...
      {[
        ...
        { label: "Days Until Conference", value: days !== null && days > 0 ? `${days}d` : "—", icon: Clock },
...
export default function Dashboard() {
  const { conferences, activeId, getActive, addConference, updateConference } = useConferenceStore();
  ...
  const openEdit   = () => { setEditing(active);  setShowModal(true); };
  ...
  return (
    <div className="flex flex-col gap-8 pb-8">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-4xl font-playfair font-bold tracking-tight">Dashboard</h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={openCreate} variant="outline" className="rounded-full gap-2">
